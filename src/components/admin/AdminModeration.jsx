          ...
          <div className={`${cardClass} p-6 space-y-4`}>
            ...
            <div className="max-h-80 overflow-y-auto space-y-2 pr-1">
              {bannedDevices.length === 0 ? (
                ...
                bannedDevices.map(b => (
                  <div key={b.device_id} className={`${theme.cardBg} border ${theme.border} p-3.5 rounded-xl text-xs space-y-1`}>
                    <div className="flex justify-between items-center">
                      <span className="font-mono text-red-500 font-extrabold flex items-center gap-1">
                        <Ban className="w-3 h-3 text-red-500" /> {b.device_id}
                      </span>
                      <button 
                        onClick={() => handleUnbanUser(b.device_id)} 
                        className="px-2.5 py-1 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 border border-emerald-500/30 rounded-lg cursor-pointer font-bold text-[11px] transition"
                      >
                        Unban
                      </button>
                    </div>
                    {b.ban_reason && (
                      <p className={`text-[11px] ${subTextClass} bg-red-500/5 p-2 rounded-lg border border-red-500/10`}>
                        <strong>Reason:</strong> {b.ban_reason}
                      </p>
                    )}
                  </div>
                ))
