    ...
    <div className={`lg:col-span-2 ${cardClass} p-6 space-y-6`}>
      ...
      <div className={`flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b ${theme.border} pb-4`}>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div 
            className={`relative flex items-center p-1 bg-slate-100 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-xl transition-all duration-300 overflow-hidden ${
          ...
          >
            ...
            <button
              ...
              className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-lg transition-colors flex-shrink-0 focus:outline-none cursor-pointer"
            ...
            <div className={`relative flex items-center gap-0 transition-all duration-300 ${
              ...
              {ranges.map((range) => (
                <button
                  ...
                  className={`w-10 h-7 rounded-lg text-xs font-black relative z-10 transition duration-300 text-center flex items-center justify-center cursor-pointer ${
                    timeRange === range
                      ? 'text-white'
            ...
            {!isFilterExpanded && (
              <span 
                ...
                className="text-xs font-black text-blue-600 dark:text-blue-400 ml-2 mr-2 select-none cursor-pointer hover:underline"
              >
                {timeRange}
              </span>
            )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-slate-50 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700/50 p-4 rounded-2xl text-center shadow-sm dark:shadow-none">
          <p className={`text-xs uppercase font-semibold ${subTextClass}`}>Peak Speed</p>
          <p className="text-2xl font-black mt-1 text-blue-500 dark:text-blue-400">{typistAnalytics.peakWpm} <span className="text-xs font-normal text-slate-400">WPM</span></p>
        </div>

        <div className="bg-slate-50 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700/50 p-4 rounded-2xl text-center shadow-sm dark:shadow-none">
          <p className={`text-xs uppercase font-semibold ${subTextClass}`}>Avg Accuracy</p>
          <p className="text-2xl font-black mt-1 text-emerald-600 dark:text-emerald-400">{typistAnalytics.avgAcc}%</p>
        </div>

        <div className="bg-slate-50 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700/50 p-4 rounded-2xl text-center shadow-sm dark:shadow-none">
          <p className={`text-xs uppercase font-semibold ${subTextClass}`}>Lessons Done</p>
          <p className="text-2xl font-black mt-1 text-purple-600 dark:text-purple-400">{typistAnalytics.completedLessonsCount}</p>
        </div>

        <div className="bg-slate-50 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700/50 p-4 rounded-2xl text-center shadow-sm dark:shadow-none">
          <p className={`text-xs uppercase font-semibold ${subTextClass}`}>Practice Time</p>
          <p className="text-2xl font-black mt-1 text-amber-600 dark:text-amber-400">{typistAnalytics.timeSpentMins} <span className="text-xs font-normal text-slate-400">mins</span></p>
        </div>
      </div>

      {/* Admin Operations Panel */}
      <div className={`p-5 border ${theme.border} rounded-2xl bg-slate-50 dark:bg-slate-900/10 space-y-4 shadow-sm dark:shadow-none`}>
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-3 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-700/60 p-2.5 rounded-2xl shadow-inner">
            <div className="flex flex-col gap-0.5">
              <span className="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Adjust Curriculum Progress</span>
              <div className="flex items-center gap-1.5">
                <input
                  ...
                  className="w-14 text-center bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-800 dark:text-white rounded-xl py-1 text-xs font-black focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
                />
                <span className="text-xs text-slate-500 dark:text-slate-300 font-black">%</span>
              </div>
            </div>
            <button
              onClick={() => handleUnlockLessons(customProgress)}
              className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-black rounded-xl transition cursor-pointer shadow-md shadow-blue-600/10 active:scale-95 border border-blue-500/20"
          ...
          <button
            ...
            className="px-4 py-3 bg-purple-650 hover:bg-purple-750 text-white dark:bg-purple-700 dark:hover:bg-purple-600 font-black rounded-2xl text-xs transition flex items-center gap-1.5 cursor-pointer shadow-lg shadow-purple-600/15 active:scale-95 border border-purple-500/30"
          >
        ...
        <div className="h-72 w-full pt-2">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={typistAnalytics.dataPoints}>
              ...
              <XAxis dataKey="date" stroke={isDarkMode ? '#94a3b8' : '#475569'} fontSize={10} tickLine={false} />
              <YAxis stroke={isDarkMode ? '#94a3b8' : '#475569'} fontSize={10} tickLine={false} domain={[0, 'auto']} />
              <Tooltip 
                contentStyle={{ 
              ...
              />
              <Area type="monotone" dataKey="wpm" stroke={isDarkMode ? '#3b82f6' : '#2563eb'} strokeWidth={3} fillOpacity={1} fill="url(#wpmGradient)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
